...
/* function setIframeHeight(iframe) {
	if (iframe) {
		...
		if (iframeWin.document.body) {
			...
			if (arguments[1]) {
				iframe.height = iframeWin.document.documentElement.scrollHeight + Number(arguments[1]) || iframeWin.document.body.scrollHeight +
					Number(arguments[1]);
			} else {
...
Vue.component("zhujian", {
	...
	template: `<li :class="{active:todo.url==tagurl}"   @click="setpUrl(todo.url)"> {{todo.url}} {{tagurl}}
				<a href="javaScript:void(0);" >{{todo.name}}  </a>
			</li>`,
	props: ['todo', ], // PROPS 事件
	mounted: function () {
...
Vue.component("zhujian1", {
	...
	/* 	<li v-for=" i in children">
			<a href="javaScript:void(0);">{{tagurl}} {{activeName}}</a>   
		</li>/* <li  v-for="item in slideBar"  :class="{active:item.url==tagurl}"    @click="setpUrl(item.url)" > 
			<a href="javaScript:void(0);"> {{item.name}}  </a>   
		</li> */

	template: `<div class="nav-tabs-custom">
		<ul class="nav nav-tabs">
		<li v-for=" item  in children" :class="{active:item.url==tagurl}"    @click="setpUrl(item.url)" >
			<a href="javaScript:void(0);"> {{item.name}}</a>   
		</li> 
	</ul>
</div>`,
	...
	beforeMount() {
		let heliostoolbar = sessionStorage.getItem("heliostoolbar");
		heliostoolbar = JSON.parse(heliostoolbar);
		this.slideBar = heliostoolbar
	}
})
